...
def generate_pathways(groups):
    ...
    for name, spec in groups:
        ...
        while abs(spec.loc[i, 'fragments m/z'] - name) < 1:
            ...
            precursor_formula = myutils.get_formula(precursor_string)
            
            #print('precursor: {}'.format(l_precursor_mass))
            
            l_spec = list(spec['fragments m/z'][i:])
            moz = np.round(np.asarray(l_spec) * 1000).astype(int)
            expanded_moz = np.zeros(np.amax(moz) + 1)
            expanded_moz[moz] = 1
            
            expanded_moz = np.convolve(expanded_moz, np.ones((1,)), mode='valid')
            ...
            for loss, loss_mass in config.neutral_losses.items():
                ...
                for mul in range(1): #upto 5 neutral losses of one kind
            ...
            while not pathways.empty():
                ...
                if is_core and expanded_moz[mol["CoreMass"]] == 1 and myutils.is_path_valid(mol["path"], precursor_formula):
                    new_mol = copy.deepcopy(mol)
                    #print(new_mol)
                    key = ""
                    ...
                    if key not in core_cand:
                        if not myutils.is_path_valid(new_mol["path"], precursor_formula):
                            print(print("{} is not valid for precursor {}".format(new_mol["path"], myutils.get_string_from_formula(precursor_formula))))
            ...
            tmp_o_count = myutils.get_formula(spec.loc[i, 'Chemical formula'])['O']
            ...
            for key, nl_row in core_cand.items():
                core_str = myutils.get_core_string_2(nl_row["path"], precursor_formula)
                
                nl_row["path"] = myutils.get_fragment_seq_from_nloss(nl_row["path"], precursor_formula)
