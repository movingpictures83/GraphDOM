...
def core_dist_over_precursor(pathway_dict, x_axis='pre_mz'):
    ...
    for pre, core_dict in pathway_dict.items():
        pre_mass_num_core.append([myutils.get_mass(myutils.get_formula(pre))/1000, len(core_dict), pre])
...
def core_dist_over_oxygen_class(pathway_dict):
    ...
    for pre, core_dict in pathway_dict.items():
        if 'N' not in pre:
            o_count[myutils.get_formula(pre)["O"]] += len(core_dict)
...
def write_cytoscape_family_graph(family_dict):
    ...
    for prec_id, family_group in family_dict.items():
        ...
        for f in range(len(family_pres) - 1, 0, -1):
            g_set.add((family_pres[f], family_pres[f-1], myutils.string_diff(family_pres[f], family_pres[f-1]), family_id))
...
def family_parents_vs_oxygen_class(family_dict):
    ...
    for pre in parents:
        if 'N' not in pre:
            o_count[myutils.get_formula(pre)["O"]] += 1
